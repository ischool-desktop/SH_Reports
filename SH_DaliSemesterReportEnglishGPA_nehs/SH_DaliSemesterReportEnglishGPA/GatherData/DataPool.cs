...
namespace SH_DaliSemesterReportEnglishGPA_nehs
{
    ...
    {   
        ...
        //  科目名稱中英文對照表
        private Dictionary<string, string> _SubjectEnglishNameMappingTable;
        //  國籍中英文對照表
        private Dictionary<string, string> _NationalityMappingTable;
        //  成績等第對照表
        private List<dynamic> _ScoreDegreeMappingTable;
        //  單一學生畢業及離校資料
        private Dictionary<string, SHLeaveInfoRecord> _dicLeaveInfos;
        //  科別中英文對照
        private Dictionary<string, string> _dicDeptNameMappingInfo;
        ...
        private void InitializeData(IEnumerable<string> pStudentIDs) 
        {
            ...
            //  科目名稱中英文對照表
            _SubjectEnglishNameMappingTable = new Dictionary<string,string>();
            QueryHelper queryHelper = new QueryHelper();
            DataTable dataTable = new DataTable();

            try
            {
                dataTable = queryHelper.Select("select content from list where name='科目中英文對照表'");
            }
            catch 
            {
                throw new Exception("查詢科目中英文對照失敗！");
            }
            if (dataTable.Rows.Count > 0 && (dataTable.Rows[0][0] + "").Length>0)
            {
                //<Content>
                //    <Subject Chinese="國文 " English="Chinese"/>
                //    <Subject Chinese="英文" English="English"/>
                //    <Subject Chinese="數學" English="Mathematics "/>
                //</Content>
                try
                {
                    XElement xElement = XElement.Parse(dataTable.Rows[0][0] + "");
                    List<XElement> xSubjects = xElement.Elements("Subject").ToList();
                    xSubjects.ForEach((x) =>
                    {
                        string subject_chinese = x.Attributes("Chinese").ElementAt(0).Value.Trim().ToUpper();
                        string subject_english = x.Attributes("English").ElementAt(0).Value;
                        if (!_SubjectEnglishNameMappingTable.ContainsKey(subject_chinese))
                            _SubjectEnglishNameMappingTable.Add(subject_chinese, subject_english);
                    });
                }
                catch 
                {
                    throw new Exception("解析科目中英文對照失敗！");
                }
            }
            //  國籍中英文對照表
            _NationalityMappingTable = new Dictionary<string, string>();
            List<UDT.NationalityMapping> nationality_mappings = (new AccessHelper()).Select<UDT.NationalityMapping>();
            if (nationality_mappings != null && nationality_mappings.Count > 0)
            {
                foreach (UDT.NationalityMapping nationality_mapping in nationality_mappings)
                    if (!_NationalityMappingTable.ContainsKey(nationality_mapping.Name.Trim().ToUpper()))
                        _NationalityMappingTable.Add(nationality_mapping.Name.Trim().ToUpper(), nationality_mapping.EnglishName);
            }
            try
            {
                XElement xElement = XElement.Parse(Properties.Resources.Nationality);
                List<XElement> xNationalities = xElement.Elements("Nationality").ToList();
                xNationalities.ForEach((x) =>
                {
                    string nationality_chinese = x.Value.Trim().ToUpper();
                    string nationality_english = x.Attributes("English").ElementAt(0).Value;
                    if (!_NationalityMappingTable.ContainsKey(nationality_chinese))
                        _NationalityMappingTable.Add(nationality_chinese, nationality_english);
                });
            }
            catch
            {
                //throw new Exception("解析國籍中英文對照失敗！");
            }
            //  成績等第對照表
            _ScoreDegreeMappingTable = new List<dynamic>();
            List<UDT.ScoreDegreeMapping> score_degree_mappings = (new AccessHelper()).Select<UDT.ScoreDegreeMapping>();
            if (score_degree_mappings != null && score_degree_mappings.Count > 0)
            {
                ...
                {
                    decimal maxScore = score_degree_mapping.MaxScore;

                    // 因當最大值100分特別修改。
                    if (maxScore == 100)
                        maxScore = 100.1m;

                    _ScoreDegreeMappingTable.Add(new { MinScore = score_degree_mapping.MinScore, MaxScore = maxScore, Degree = score_degree_mapping.Degree, GPA = score_degree_mapping.GPA });
                }                    
            }
            //  畢業及離校資訊
            _dicLeaveInfos = new Dictionary<string, SHLeaveInfoRecord>();
            List<SHLeaveInfoRecord> LeaveInfoRecords = SHLeaveInfo.SelectByStudentIDs(pStudentIDs);
            if (LeaveInfoRecords != null && LeaveInfoRecords.Count > 0)
                _dicLeaveInfos = LeaveInfoRecords.ToDictionary(x => x.RefStudentID);
            //  科別中英文對照
            _dicDeptNameMappingInfo = new Dictionary<string, string>();
            try
            {
                dataTable = queryHelper.Select("select content from list where name='SchoolConfig'");
            }
            catch
            {
            ...
            if (dataTable.Rows.Count > 0 && (dataTable.Rows[0][0] + "").Length > 0)
            {
                //<Content>
                //    <科別中英文對照表>
                //        <Department Chinese="普通科" English="普通科英文名字"/>
                //        <Department Chinese="應用日語學程-99" English="Test1"/>
                //        <Department Chinese="學術自然學程" English="學術自然學程'英文"/>
                //        <Department Chinese="資訊科" English="ID"/>
                //    </科別中英文對照表>
                // </Content>
                try
                {
                    XElement xElement = XElement.Parse(dataTable.Rows[0][0] + "");
                    List<XElement> xDepts = xElement.Descendants("Department").ToList();
                    xDepts.ForEach((x) =>
                    {
                        string dept_chinese = x.Attribute("Chinese").Value;
                        string dept_english = x.Attribute("English").Value;
                        if (!_dicDeptNameMappingInfo.ContainsKey(dept_chinese))
                            _dicDeptNameMappingInfo.Add(dept_chinese, dept_english);
                    });
                }
                catch
                {
                    //throw new Exception("解析科別中英文對照失敗！");
                }
            }

        }

        public Dictionary<string, string> GetDeptNameMappingInfo()
        {
            return this._dicDeptNameMappingInfo;
        }

        public SHLeaveInfoRecord GetLeaveInfo(string pStudentID)
        {
            if (_dicLeaveInfos.ContainsKey(pStudentID))
                return _dicLeaveInfos[pStudentID];
            else
                return null;
        }

        public List<dynamic> GetScoreDegreeMapping()
        {
            return this._ScoreDegreeMappingTable;
        }

        public Dictionary<string, string> GetSubjectEnglishNameMapping()
        {
            return this._SubjectEnglishNameMappingTable;
        }

        public Dictionary<string, string> GetNationalityEnglishNameMapping()
        {
            return this._NationalityMappingTable;
        }
         ...
         ***********************************/
        public bool RetainInTheSameGrade(string pStudentID, int pSchoolYear)
        {
            string key = pStudentID + "_" + pSchoolYear.ToString();
            decimal? totalCredit = 0;
            decimal? notPassTotalCredit = 0;
            int totalSubjectAmount = 0;
            int notPassTotalSubjectAmount = 0;
            bool hasZeroScoreSubject = false;
            decimal yearEntryScore = GetYearEntryScore(pStudentID, pSchoolYear, SHYearSubjectScoreInfo.Entry.學業);

            List<SHSubjectYearScoreInfo> _YearSubjectScoreInfo = _SHYearSubjectScoreInfo.GetPersonalYearSubjectScoreInfo(pStudentID, pSchoolYear);
            ...
            if (((notPassTotalCredit * 2) <= totalCredit) && (!hasZeroScoreSubject) && (GetYearEntryScore(pStudentID, pSchoolYear, SHYearSubjectScoreInfo.Entry.學業) >= (GetPassingStandard(pStudentID, this.GetPersonalYearSubjectScoreHistoryInfo(pStudentID)[pSchoolYear]))))
                return false;

            //  剛好 1/2：升級
            //  學年成績不及格之科目未超過修習全部科目二分之一，且其不及格之科目成績無零分，而學年總平均成績及格
            if (((notPassTotalSubjectAmount * 2) <= totalSubjectAmount) && (!hasZeroScoreSubject) && (GetYearEntryScore(pStudentID, pSchoolYear, SHYearSubjectScoreInfo.Entry.學業) >= (GetPassingStandard(pStudentID, this.GetPersonalYearSubjectScoreHistoryInfo(pStudentID)[pSchoolYear]))))
                return false;
