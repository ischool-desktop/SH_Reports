...
namespace SH_DaliSemesterReportEnglishGPA_nehs
{
    ...
    {
        //MemoryStream selectedTemplate;  //  英文成績單(GPA)樣版
        int templateNumber;             //  樣版代碼：1-->預設，2-->自訂。
        string text1;                   //  學籍表字
        string text2;                   //  學籍表號
        ...
        public List<DataSet> TransferFormat(object[] bgwObject)
        {
            ...
            retakeNumber = (int)bgwObject[19];
            scoreFormatNumber = (int)bgwObject[20];
            updateRecordNumber = (int)bgwObject[21];
            GPAText = (string)bgwObject[22];
            HGPAText = (string)bgwObject[23];
            CGPAText= (string)bgwObject[24];
            return Transfer();
        }
        ...
        private void ProduceSHSemesterScoreData(SHStudentRecord pStudent, DataSet schoolRollTable, int yIndex, int pSchoolYear, int pSemester, DataPool dataPool, List<SHSubjectSemesterScoreInfo> pSHSubjectSemesterScoreInfo)
        {
            ...
            foreach (SHSubjectSemesterScoreInfo ss in pSHSubjectSemesterScoreInfo)
            {
                ...
                //  重修成績顯示方式：1：併入擇優成績、2：擇優成績右側
                //  成績格式：1：分數、2：等第
                if (ss.ReCourseScore.HasValue && retakeNumber == 1)
                {
                    if (scoreBetter.HasValue)
                    {
                        if (ss.ReCourseScore.Value > scoreBetter.Value)
                        {
                            scoreBetter = ss.ReCourseScore.Value;
                            scoreSign = retakeSign;
                        }
                    }
                    else
                    {
        ...
        private void ProduceSHYearScoreData(SHStudentRecord pStudent, DataSet schoolRollTable, int yIndex, List<SHSubjectYearScoreInfo> pSHSubjectYearScoreInfo)
        {
            ...
            //  原始成績
            List<dynamic> ScoreDegreeMappings = dataPool.GetScoreDegreeMapping();
            schoolRollTable.Tables.Add(pSHSubjectYearScoreInfo.Select((x) =>
            {
                if (!x.Score.HasValue)
                    return "";
        ...
        private DataSet GetSchoolRollTable(SHStudentRecord student, DataPool dataPool)
        {
            ...
            // 校名
            schoolRollTable.Tables.Add(School.EnglishName.ToDataTable("SchoolName", "校名"));
            ...
            // 學生基本資料：英文姓名
            schoolRollTable.Tables.Add(student.EnglishName.ToDataTable("StudentEnglishName", "學生英文姓名"));

            // 學生基本資料：性別
            schoolRollTable.Tables.Add((student.Gender == "男" ? "MALE" : "FEMALE").ToDataTable("Gender", "性別"));
            Thread.CurrentThread.CurrentCulture = new CultureInfo("en-US");
            // 學生基本資料：出生日期
            string birthday = student.Birthday.HasValue ? student.Birthday.Value.ToString("MMMM d, yyyy") : string.Empty;
            ...
            //}
            schoolRollTable.Tables.Add(birthday.ToDataTable("Birthday", "出生日期", DateTime.Today.GetType()));
            // 學生基本資料：學號
            ...
            if (dataPool.GetDeptNameMappingInfo().ContainsKey(pDepartmentName))
                pDepartmentName = dataPool.GetDeptNameMappingInfo()[pDepartmentName];
            ...
            //  學生基本資料：國籍
            string nationality = student.Nationality.Trim().ToUpper();
            Dictionary<string, string> dicNationalityMappings = dataPool.GetNationalityEnglishNameMapping();
            if (dicNationalityMappings.ContainsKey(nationality))
                schoolRollTable.Tables.Add(dicNationalityMappings[nationality].ToDataTable("EnglishNationality", "國籍英文"));
            else
                schoolRollTable.Tables.Add(student.Nationality.ToDataTable("EnglishNationality", "國籍英文"));
            ...
            if (_PersonalSemesterSubjectScoreHistoryInfo != null && _PersonalSemesterSubjectScoreHistoryInfo.Count > 0)
            {
                ...
                foreach (int gradeYear in _PersonalSemesterSubjectScoreHistoryInfo.Keys)
                {
                    ...
                    {
                        allSchoolYearSemesterPair = allSchoolYearSemesterPair.OrderBy(x => x.Key).ThenBy(x => x.Value).ToList();
                        schoolYear = allSchoolYearSemesterPair.Select(x => x.Key).Distinct().ToList();
                        schoolRollTable.Tables.Add((allSchoolYearSemesterPair.ElementAt(0).Key + 1911 + ".9").ToDataTable("Y" + yIndex.ToString() + "S1YearMonth", "第" + yIndex + "學年日期區間", DateTime.Today.GetType()));
                        if (allSchoolYearSemesterPair.Count > 1)
                            schoolRollTable.Tables.Add((allSchoolYearSemesterPair.ElementAt(allSchoolYearSemesterPair.Count - 1).Key + 1912 + ".7").ToDataTable("Y" + yIndex.ToString() + "S2YearMonth", "第" + yIndex + "學年日期區間", DateTime.Today.GetType()));
                    }
                    string sYear = string.Empty;
                    foreach (int s in schoolYear)
                    {
                        sYear += s.ToString() + "/";
                    }
                ...
                Dictionary<string, string> dicSubjectEnglishNameMappings = dataPool.GetSubjectEnglishNameMapping();
                foreach (int gradeYear in _PersonalSemesterSubjectScoreHistoryInfo.Keys)
                {
                    yIndex = gradeYear;
                    string prefix = "Y" + yIndex + "S";
                    List<string> mergeSubjects = new List<string>();
                    List<string> english_MergeSubjects = new List<string>();
                    ...
                    if (schoolRollTable.Tables.Contains(prefix + "1Subject") || schoolRollTable.Tables.Contains(prefix + "2Subject"))
                    {
                        ...
                        string english_MergeSubject = string.Empty;
                        int j = 0;
                        if (schoolRollTable.Tables.Contains(prefix + "1Subject"))
                        {
                            foreach (DataRow dr in schoolRollTable.Tables[prefix + "1Subject"].Rows)
                            {
                                ...
                                mergeSubject = subjectName;// +" " + (string.IsNullOrWhiteSpace(Util.NumberToRomanChar(s1Level)) ? "" : Util.NumberToRomanChar(s1Level) + ",") + Util.NumberToRomanChar(s2Level);

                                if (mergeSubject.EndsWith(","))
                                    mergeSubject = mergeSubject.Substring(0, mergeSubject.Length - 1);

                                mergeSubjects.Add(mergeSubject);
                                if (dicSubjectEnglishNameMappings.ContainsKey(subjectName.Trim().ToUpper()))
                                    english_MergeSubjects.Add(mergeSubject.Replace(subjectName, dicSubjectEnglishNameMappings[subjectName.Trim().ToUpper()]));
                                else
                                    english_MergeSubjects.Add(mergeSubject);
                                j++;
                            }
                        }
                        else
                        {
                            if (schoolRollTable.Tables.Contains(prefix + "2Subject"))
                            {
                                ...
                                {
                                    string subjectName = schoolRollTable.Tables[prefix + "2Subject"].Rows[k]["科目名稱"].ToString();
                                    mergeSubject = subjectName;// +" " + Util.NumberToRomanChar(schoolRollTable.Tables[prefix + "2Level"].Rows[k]["級別"].ToString());
                    ...
                    schoolRollTable.Tables.Add(english_MergeSubjects.ToDataTable("Y" + yIndex.ToString() + "EnglishMergedSubject", "合併級別英文科目名稱"));
                }
            }
            #endregion
            ...
            StudentAvgScore sas = new StudentAvgScore();
            sas.StudentID = student.ID;
            sas.StudentNumber = student.StudentNumber;
            sas.Name = student.Name;
            if (student.SeatNo.HasValue)
                sas.SeatNo = student.SeatNo.Value.ToString();
            sas.ClassName = student.Class.Name;
            sas.AvgSSScore = 0;
            decimal sum = 0;
            decimal cot = 0;

            List<string> ssString = new List<string>();
            List<SHSemesterEntryScoreRecord> sssList = (from data in _SHSemesterEntryScoreRecords orderby data.SchoolYear descending select data).ToList();
            
            foreach (SHSemesterEntryScoreRecord sr in sssList)
            {
                string skey = "" + sr.Semester + sr.GradeYear;
                
                if(!ssString.Contains(skey))
            ...
            if(cot>0)
            {
                sas.AvgSSScore =sum / cot;
                Gobal.xlsStudentAvgScoreList.Add(sas);
            }
            ...
            if (_SHSemesterEntryScoreRecords.Count > 0 && _SHSemesterEntryScoreHistory.Count > 0)
            {
                ...
                foreach (SHSemesterEntryScoreRecord sr in _SHSemesterEntryScoreRecords)
                {
                    ...
                    //        foreach (dynamic o in ScoreDegreeMappings)
                    //        {
                    //            if (sr.Scores["體育"] >= o.MinScore && sr.Scores["體育"] <= o.MaxScore)
                    //            {
                    //                schoolRollTable.Tables.Add(o.Degree.ToDataTable(prefix + "PhysicalScore", "體育成績"));
                    //                break;
                    //            }
                    //        }
                    //    }
                    ...
                    //    foreach (dynamic o in ScoreDegreeMappings)
                    //    {
                    //        if (sr.Scores["健康與護理"] >= o.MinScore && sr.Scores["健康與護理"] <= o.MaxScore)
                    //        {
                    ...
                    //    foreach (dynamic o in ScoreDegreeMappings)
                    //    {
                    //        if (sr.Scores["國防通識"] >= o.MinScore && sr.Scores["國防通識"] <= o.MaxScore)
                    //        {
                    ...
                    //    foreach (dynamic o in ScoreDegreeMappings)
                    //    {
                    //        if (sr.Scores["實習科目"] >= o.MinScore && sr.Scores["實習科目"] <= o.MaxScore)
                    //        {
                    ...
                    //    foreach (dynamic o in ScoreDegreeMappings)
                    //    {
                    //        if (sr.Scores["專業科目"] >= o.MinScore && sr.Scores["專業科目"] <= o.MaxScore)
                    //        {
            ...
            if (_PersonalYearSubjectScoreHistoryInfo != null)
            {
                ...
                {
                    bool retainInTheSameGrade = dataPool.RetainInTheSameGrade(student.ID, kv.Key);

                    schoolRollTable.Tables.Add((retainInTheSameGrade ? "重讀" : "升級").ToDataTable("Y" + kv.Value.ToString() + "RetainInTheSameGrade", "升級或應重讀判斷"));
                }
            }
            ...
            if (shurrs != null && shurrs.Count > 0)
            {
                ...
                var enrollUpdateRecords = from updateRecords in shurrs
                                          where ((updateRecords.UpdateCode.Length > 0 && updateRecords.UpdateCode.Substring(0, 1) == "0") || ((updateRecordNumber == 4) && updateRecords.UpdateCode.Length > 2 && updateRecords.UpdateCode.Substring(0, 3) == "101") || ((updateRecordNumber == 3) && updateRecords.UpdateCode.Length > 2 && updateRecords.UpdateCode.Substring(0, 3) == "111"))
                                          orderby updateRecords.UpdateDate descending
                                          select updateRecords;

                DataTable EnrollRecordTable = new DataTable("EntranceDate");
                EnrollRecordTable.Columns.Add("EntranceDate", DateTime.Today.GetType());
                foreach (SHUpdateRecordRecord u in enrollUpdateRecords)
                {
                    ...
                    if (DateTime.TryParse(u.UpdateDate, out UpdateDate))
                        pRow["EntranceDate"] = UpdateDate.ToString("MMMM d, yyyy");
                    else
                        pRow["EntranceDate"] = "";
                ...
                DataTable GraduateRecordTable = new DataTable("GraduationDate");
                GraduateRecordTable.Columns.Add("GraduationDate", DateTime.Today.GetType());
                //DataTable UnderGraduateRecordTable = new DataTable("UnderGraduateDate");
                //UnderGraduateRecordTable.Columns.Add("UnderGraduateDate", DateTime.Today.GetType());

                foreach (SHUpdateRecordRecord u in graduateUpdateRecords)
                {
                    DataRow pRow = GraduateRecordTable.NewRow();
                    //DataRow qRow = UnderGraduateRecordTable.NewRow();

                    DateTime UpdateDate;
                    string updateDate = string.Empty;
                    if (DateTime.TryParse(u.UpdateDate, out UpdateDate))
                        updateDate = UpdateDate.ToString("MMMM d, yyyy");

                    pRow["GraduationDate"] = updateDate;
                    GraduateRecordTable.Rows.Add(pRow);

                    //qRow["UnderGraduateDate"] = updateDate;
                    //UnderGraduateRecordTable.Rows.Add(qRow);

                    break;
                }
                schoolRollTable.Tables.Add(GraduateRecordTable);
                //schoolRollTable.Tables.Add(UnderGraduateRecordTable);

                //SHLeaveInfoRecord leaveInfoRecord = dataPool.GetLeaveInfo(student.ID);
                //if (leaveInfoRecord == null || leaveInfoRecord.Reason == null || !leaveInfoRecord.Reason.Equals("畢業"))
                //{
                //    if (schoolRollTable.Tables.Contains("GraduationDate"))
                //        schoolRollTable.Tables.Remove("GraduationDate");
                //}
                //else
                //{
                //    if (schoolRollTable.Tables.Contains("UnderGraduateDate"))
                //        schoolRollTable.Tables.Remove("UnderGraduateDate");
                //}
                ...
                //{
                //    if ((updateRecordNumber == 3) && u.UpdateCode.Substring(0, 3) == "111")
                //        continue;

                //    if ((updateRecordNumber == 4) && u.UpdateCode.Substring(0, 3) == "101")
                //        continue;
            ...
            schoolRollTable.Tables.Add(DateTime.Today.ToString("MMMM d, yyyy").ToDataTable("PrintDate", "報表列印日期", DateTime.Today.GetType()));

            #endregion

            #region 實中GPA文字
            
            DataTable dtGPAText = new DataTable("GPA");
            dtGPAText.Columns.Add("GPA");
            DataRow GPATextR = dtGPAText.NewRow();
            GPATextR["GPA"] = GPAText;
            dtGPAText.Rows.Add(GPATextR);
            schoolRollTable.Tables.Add(dtGPAText);

            DataTable dtHGPAText = new DataTable("HGPA");
            dtHGPAText.Columns.Add("HGPA");
            DataRow HGPATextR = dtHGPAText.NewRow();
            HGPATextR["HGPA"] = HGPAText;
            dtHGPAText.Rows.Add(HGPATextR);
            schoolRollTable.Tables.Add(dtHGPAText);

            DataTable dtCGPAText = new DataTable("CGPA");
            dtCGPAText.Columns.Add("CGPA");
            DataRow CGPATextR = dtCGPAText.NewRow();
            CGPATextR["CGPA"] = CGPAText;
            dtCGPAText.Rows.Add(CGPATextR);
            schoolRollTable.Tables.Add(dtCGPAText);
