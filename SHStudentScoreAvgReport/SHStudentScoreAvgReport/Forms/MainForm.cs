...
namespace SHStudentScoreAvgReport.Forms
{
    public partial class MainForm : BaseForm
    {
        List<string> _ClassIDList;
        Dictionary<string, Dictionary<string,StudentScore>> _ClassStudentScoreDict;
        Dictionary<string, List<DataRow>> _StudentSubjScoreRowDict;
        Dictionary<string, List<DataRow>> _StudentEntryScoreRowDict;
        ...
        public MainForm(List<string> ClassIDList)
        {
            ...
            _StudentEntryScoreRowDict = new Dictionary<string, List<DataRow>>();
            _StudentSubjScoreRowDict = new Dictionary<string, List<DataRow>>();
        ...
        {
            FISCA.Presentation.MotherForm.SetStatusBarMessage("高中各育成績總平均一覽表產生中", e.ProgressPercentage);
        }
        ...
        void _bgWork_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
           // 匯出報表
            if(_wbRpt !=null)
                Utility.CompletedXls("高中各育成績總平均一覽表", _wbRpt);
        ...
        void _bgWork_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            // 取得成績(科目、分項)
            _StudentSubjScoreRowDict = Utility.GetStudentSemsSubjScoreByStudentID(_StudentIDList);
            _bgWork.ReportProgress(20);
            _StudentEntryScoreRowDict = Utility.GetStudentSemsEntryScoreByStudentID(_StudentIDList);
            ...
            foreach (string className in _ClassStudentScoreDict.Keys)
            {
                ...
                {                    
                    ...
                    {
                        // 分項
                        if(_StudentEntryScoreRowDict.ContainsKey(studID))
                        {
                            _ClassStudentScoreDict[className][studID].AddEnteryScore("智育", Cal.CalEntryAvgScore("學業", _StudentEntryScoreRowDict[studID], true));
                            //_ClassStudentScoreDict[className][studID].AddEnteryScore("智育(原始)", Cal.CalEntryAvgScore("學業", _StudentEntryScoreRowDict[studID], false));
                            //_ClassStudentScoreDict[className][studID].AddEnteryScore("體育", Cal.CalEntryAvgScore("體育", _StudentEntryScoreRowDict[studID], true));
                            //_ClassStudentScoreDict[className][studID].AddEnteryScore("體育(原始)", Cal.CalEntryAvgScore("體育", _StudentEntryScoreRowDict[studID], false));
                        }

                        // 科目
                        if(_StudentSubjScoreRowDict.ContainsKey(studID))
                        {
                            _ClassStudentScoreDict[className][studID].AddEnteryScore("體育",Cal.CalSubjGroupAvgScore(SubjNameList2, _StudentSubjScoreRowDict[studID], true));
                            //_ClassStudentScoreDict[className][studID].AddEnteryScore("體育(原始)", Cal.CalSubjGroupAvgScore(SubjNameList2, _StudentSubjScoreRowDict[studID], false));
                            _ClassStudentScoreDict[className][studID].AddEnteryScore("美育", Cal.CalSubjGroupAvgScore(SubjNameList3, _StudentSubjScoreRowDict[studID], true));
                            //_ClassStudentScoreDict[className][studID].AddEnteryScore("美育(原始)", Cal.CalSubjGroupAvgScore(SubjNameList3, _StudentSubjScoreRowDict[studID], false));
                        }
                    }            
                }
        ...
        { 
            // 讀取樣版
            _wbRpt= new Workbook(new MemoryStream(Properties.Resources.高中各育成績總平均樣版));           
        ...
        private void MainForm_Load(object sender, EventArgs e)
        {
            ...
            sb.AppendLine("智育：讀取學生各學期學業分項成績，成績計算使用算數平均。");
