...
namespace SH_StudentRecordReport
{
    ...
    {
        //MemoryStream selectedTemplate;  //  成績證明書樣版
        int templateNumber;             //  樣版代碼：1，2-->高中。3，4-->高職。5，6-->進校
        string text1;                   //  成績證明書字
        string text2;                   //  成績證明書號
        string text3;                   //  成績證明書輸出路徑，暫時不用
        ...
        /// <summary>
        /// 範例程式：收集成績證明書相關資料。請注意：請儘量減少取得資料的 IO
        /// </summary>
        /// <param name="studentID">學生系統編號</param>
        /// <returns>個人成績證明書相關資料</returns>
        private DataSet GetSchoolRollTable(SHStudentRecord student, DataPool dataPool)
        {
            ...
            if (shurrs != null && shurrs.Count > 0)
            {
                ...
                //  異動代碼首碼為「0」者：新生異動
                //  異動代碼前3碼為「101」且為進校成績證明書：它校轉入
                //  異動代碼前3碼為「111」且為日校、高職成績證明書：它校轉入
