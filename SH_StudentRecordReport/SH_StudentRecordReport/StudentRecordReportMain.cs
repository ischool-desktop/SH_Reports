...
namespace SH_StudentRecordReport
{
    ...
    {        
        ...
        {
            reportName = "歷年成績單 (可選擇列印原始/擇優)";
            reportPath = "資料統計";

            //  學生頁籤-->成績相關報表-->歷年成績單 (可選擇列印原始/擇優)
            button_student = RoleAclSource.Instance["學生"]["報表"];
            button_student.Add(new RibbonFeature("Student_Button_SH_StudentRecordReport", "歷年成績單 (可選擇列印原始/擇優)"));
            ...
            //  班級頁籤-->成績相關報表-->歷年成績單 (可選擇列印原始/擇優)
            button_class = RoleAclSource.Instance["班級"]["報表"];
            button_class.Add(new RibbonFeature("Class_Button_SH_StudentRecordReport", "歷年成績單 (可選擇列印原始/擇優)"));
        ...
        private void Clicked()
        {
            ...
            if (form.ShowDialog() == DialogResult.OK)
            {
                ...
                try
                {
                    ...
                    MotherForm.SetStatusBarMessage("歷年成績單 (可選擇列印原始/擇優)產生中...");
        ...
        private void Print()
        {
            ...
            {
                string message = "下列學生沒有學號，請補齊資料再列印成績證明書：\n";
            ...
            if (saveFileType == 1)
            {
                ...
                {
                    string message = "下列學生沒有設定班級，無法以「每個班級儲存一個檔案」的方式產生成績證明書：\n";
        ...
        private void _BGWStudentRecord_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            {
                MessageBox.Show("成績證明書產生完成。", "成績證明書", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1, MessageBoxOptions.DefaultDesktopOnly);
        ...
        {
            MotherForm.SetStatusBarMessage("歷年成績單 (可選擇列印原始/擇優)產生中...", e.ProgressPercentage);
        }
